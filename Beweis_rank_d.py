...
class Beweis:
    def __init__(self, rank, points, collinearities, name, maxiter = 11):
        ...
        self.phis = [comb for comb in combinations(points, self.rank)
        ...
        self.refpoints = [comb for comb in combinations(points, self.rank-2)]
        self.cross_ratios = set()
        for x in self.refpoints:
            for a, b, c, d in permutations(points - set(x), 4):
    ...
    def init_GPR(self):
        ...
        for coll in self.collinearities:
            ...
            for c in combinations(coll, self.rank-2):  # all possible viewpoints (d-2) elements
                a,b = [l for l in coll if l not in c]
